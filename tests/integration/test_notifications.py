...
from tuesmon.projects.notifications import services
from tuesmon.projects.notifications import models
from tuesmon.projects.notifications.choices import NotifyLevel
from tuesmon.projects.notifications.choices import WebNotificationType
from tuesmon.projects.history.choices import HistoryType
from tuesmon.projects.history.services import take_snapshot
from tuesmon.permissions.choices import MEMBERS_PERMISSIONS
from tuesmon.users.gravatar import get_user_gravatar_id
...
def test_send_notifications_using_services_method_for_user_stories(settings, mail):
    ...
    for msg in mail.outbox:
        ...
        list_id = "Tuesmon/{p_name} <tuesmon.{p_slug}@{domain}>" \
...
def test_task_updated_generates_web_notifications(client):
    ...
    client.login(member1.user)
    mock_path = "tuesmon.projects.tasks.api.TaskViewSet.pre_conditions_on_save"
    with patch(mock_path):
        client.patch(
...
def test_us_updated_generates_web_notifications(client):
    ...
    client.login(member1.user)
    mock_path = "tuesmon.projects.userstories.api.UserStoryViewSet." \
