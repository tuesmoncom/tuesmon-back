...
def test_auth_url(client):
    ...
    with mock.patch('tuesmon.importers.trello.importer.OAuth1Session') as OAuth1SessionMock:
        session = mock.Mock()
    ...
    assert response.data['url'] == "https://trello.com/1/OAuthAuthorizeToken?oauth_token=token&scope=read,write,account&expiration=1day&name=Tuesmon&return_url=http://localhost:9001/project/new/import/trello"
...
def test_import_trello_list_users(client, settings):
    ...
    with mock.patch('tuesmon.importers.trello.api.TrelloImporter') as TrelloImporterMock:
        instance = mock.Mock()
...
def test_import_trello_list_users_with_problem_on_request(client, settings):
    ...
    with mock.patch('tuesmon.importers.trello.importer.TrelloClient') as TrelloClientMock:
        instance = mock.Mock()
...
def test_import_trello_project_without_project_id(client, settings):
    ...
    with mock.patch('tuesmon.importers.trello.tasks.TrelloImporter') as TrelloImporterMock:
        response = client.post(url, content_type="application/json", data=json.dumps({"token": "token"}))
