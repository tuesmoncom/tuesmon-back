...
def test_authorize(client, settings):
    ...
    with mock.patch('tuesmon.importers.github.api.GithubImporter') as GithubImporterMock:
...
def test_import_github_list_users_with_problem_on_request(client):
    ...
    with mock.patch('tuesmon.importers.github.importer.GithubClient') as GithubClientMock:
        instance = mock.Mock()
...
def test_import_github_project_without_project_id(client, settings):
    ...
    with mock.patch('tuesmon.importers.github.tasks.GithubImporter') as GithubImporterMock:
        response = client.post(url, content_type="application/json", data=json.dumps({"token": "token"}))
