...
def test_auth_url(client, settings):
    ...
    with mock.patch('tuesmon.importers.asana.api.AsanaImporter') as AsanaImporterMock:
...
def test_import_asana_list_users_with_problem_on_request(client):
    ...
    with mock.patch('tuesmon.importers.asana.importer.AsanaClient') as AsanaClientMock:
        instance = mock.Mock()
...
def test_import_asana_project_without_project_id(client, settings):
    ...
    with mock.patch('tuesmon.importers.asana.tasks.AsanaImporter') as AsanaImporterMock:
        response = client.post(url, content_type="application/json", data=json.dumps({"token": "token"}))
