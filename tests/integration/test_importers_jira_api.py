...
from tuesmon.users.models import AuthData
...
def test_auth_url(client):
    ...
    with mock.patch('tuesmon.importers.jira.api.JiraNormalImporter') as JiraNormalImporter:
...
def test_import_jira_list_users(client, settings):
    ...
    with mock.patch('tuesmon.importers.jira.api.JiraNormalImporter') as JiraNormalImporterMock:
        instance = mock.Mock()
...
def test_import_jira_list_users_with_problem_on_request(client, settings):
    ...
    with mock.patch('tuesmon.importers.jira.common.JiraClient') as JiraClientMock:
        instance = mock.Mock()
...
def test_import_jira_list_projects(client, settings):
    ...
    with mock.patch('tuesmon.importers.jira.api.JiraNormalImporter') as JiraNormalImporterMock:
        with mock.patch('tuesmon.importers.jira.api.JiraAgileImporter') as JiraAgileImporterMock:
            instance = mock.Mock()
...
def test_import_jira_project_without_project_id(client, settings):
    ...
    with mock.patch('tuesmon.importers.jira.tasks.JiraNormalImporter') as JiraNormalImporterMock:
        response = client.post(url, content_type="application/json", data=json.dumps({"token": "access.secret", "url": "http://jiraserver"}))
...
def test_import_jira_project_with_celery_enabled(client, settings):
    ...
    with mock.patch('tuesmon.importers.jira.api.JiraNormalImporter') as ApiJiraNormalImporterMock:
        with mock.patch('tuesmon.importers.jira.tasks.JiraNormalImporter') as TasksJiraNormalImporterMock:
