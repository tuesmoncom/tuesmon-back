...
def test_list_milestones_tuesmon_info_headers(client):
    user = f.UserFactory.create()
    project = f.ProjectFactory.create(owner=user)
    role = f.RoleFactory.create(project=project)
    f.MembershipFactory.create(project=project, user=user, role=role, is_admin=True)
    ...
    assert "tuesmon-info-total-closed-milestones" in response1["access-control-expose-headers"]
    assert "tuesmon-info-total-opened-milestones" in response1["access-control-expose-headers"]
    assert response1.has_header("Tuesmon-Info-Total-Closed-Milestones") == False
    assert response1.has_header("Tuesmon-Info-Total-Opened-Milestones") == False

    assert response2.status_code == 200
    assert "tuesmon-info-total-closed-milestones" in response2["access-control-expose-headers"]
    assert "tuesmon-info-total-opened-milestones" in response2["access-control-expose-headers"]
    assert response2.has_header("Tuesmon-Info-Total-Closed-Milestones") == True
    assert response2.has_header("Tuesmon-Info-Total-Opened-Milestones") == True
    assert response2["tuesmon-info-total-closed-milestones"] == "3"
    assert response2["tuesmon-info-total-opened-milestones"] == "1"
