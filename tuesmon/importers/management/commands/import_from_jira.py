...
from tuesmon.importers.jira.agile import JiraAgileImporter
from tuesmon.importers.jira.normal import JiraNormalImporter
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument('--project-id', dest="project_id", type=str,
                            help='Project ID or full name (ex: tuesmoncom/tuesmon-back)')
        ...
        if project_type == "project":
            ...
            for issue_type in importer.list_issue_types(project_id):
                while True:
                    ...
                    tuesmon_type = input("{}: ".format(issue_type['name']))
                    if tuesmon_type not in ['epic', 'us', 'issue']:
                        print("use a valid tuesmon type (epic, us, issue)")
                        continue

                    types_bindings[tuesmon_type].append(issue_type)
                    break
