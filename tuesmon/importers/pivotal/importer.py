...
from tuesmon.projects.references.models import recalc_reference_counter
from tuesmon.projects.models import Project, ProjectTemplate, Membership, Points
from tuesmon.projects.userstories.models import UserStory, RolePoints
from tuesmon.projects.tasks.models import Task
from tuesmon.projects.milestones.models import Milestone
from tuesmon.projects.epics.models import Epic, RelatedUserStory
from tuesmon.projects.attachments.models import Attachment
from tuesmon.projects.history.services import take_snapshot
from tuesmon.projects.history.services import (make_diff_from_dicts,
                                             make_diff_values,
...
class PivotalImporter:
    ...
    def _import_epics_data(self, project_data, project, options):
        ...
        for epic in project_data['epics']:
            ...
            tuesmon_epic = Epic.objects.create(
                project=project,
                owner=self._user,
            ...
            Epic.objects.filter(id=tuesmon_epic.id).update(
                ref=epic['id'],
            ...
            for watcher in epic['follower_ids']:
                watcher_user = users_bindings.get(watcher, None)
                if watcher_user:
                    tuesmon_epic.add_watcher(watcher_user)

            take_snapshot(tuesmon_epic, comment="", user=None, delete=False)
            self._import_comments(project_data, tuesmon_epic, epic, options)
            self._import_epic_activity(project_data, tuesmon_epic, epic, options)
            counter += 1
    ...
    def _import_epic_activity(self, project_data, tuesmon_epic, epic, options):
        offset = 0
        while True:
            activities = self._client.get(
            ...
            offset += 300
            for activity in activities['data']:
                self._import_activity(tuesmon_epic, activity, options)
    ...
    def _transform_activity_data(self, obj, activity, options):
        ...
        tuesmon_user = users_bindings.get(activity.get('performed_by', {}).get('id', None), None)
        if tuesmon_user:
