...
from tuesmon.users.models import User, AuthData

from tuesmon.importers.exceptions import InvalidAuthResult, FailedRequest
...
class GithubImporter:
    ...
    def _import_project_data(self, repo, options):
        ...
        for milestone in self._client.get("/repos/{}/milestones".format(repo['full_name'])):
            tuesmon_milestone = Milestone.objects.create(
            ...
            Milestone.objects.filter(id=tuesmon_milestone.id).update(
    ...
    def _import_issues_data(self, project, repo, options):
        ...
        while True:
            ...
            for issue in issues:
                ...
                tuesmon_issue = Issue.objects.create(
                    ref=issue['number'],
                    project=project,
                    owner=users_bindings.get(issue['user']['id'], self._user),
                    assigned_to=assigned_to,
                ...
                if len(assignees) > 1:
                    for assignee in assignees:
                        if assignee['id'] != issue.get('assignee', {}).get('id', None):
                            assignee_user = users_bindings.get(assignee['id'], None)
                            if assignee_user is not None:
                                tuesmon_issue.add_watcher(assignee_user)

                Issue.objects.filter(id=tuesmon_issue.id).update(
                    ref=issue['number'],
                    modified_date=issue['updated_at'],
                    created_date=issue['created_at']
                )

                take_snapshot(tuesmon_issue, comment="", user=None, delete=False)
    ...
    def _transform_event_data(self, obj, event, options, cumulative_data):
        ...
        tuesmon_user = users_bindings.get(event['actor']['id'], None) if event['actor'] else None
        if tuesmon_user:
        ...
        if event['event'] == "renamed":
            ...
            if isinstance(obj, UserStory):
                ...
                try:
                    tuesmon_milestone = obj.project.milestones.get(name=event['milestone']['title'])
                    cumulative_data["milestone"] = tuesmon_milestone.id
                    cumulative_data['milestone_name'] = tuesmon_milestone.name
                except Milestone.DoesNotExist:
