...
from tuesmon.projects.models import Project, ProjectTemplate, Points
        ...
        for issue_type in types:
            ...
            while True:
                ...
                for issue in issues['issues']:
                    ...
                    self._import_to_custom_fields(tuesmon_issue, issue, options)

                    tuesmon_issue.ref = issue['key'].split("-")[1]
                    Issue.objects.filter(id=tuesmon_issue.id).update(
                        ref=tuesmon_issue.ref,
                        modified_date=issue['fields']['updated'],
                        created_date=issue['fields']['created']
                    )
                    ...
                    for subtask in issue['fields']['subtasks']:
                        print("WARNING: Ignoring subtask {} because parent isn't a User Story".format(subtask['key']))
                    self._import_comments(tuesmon_issue, issue, options)
                    self._import_attachments(tuesmon_issue, issue, options)
                    self._import_changelog(project, tuesmon_issue, issue, options)
                    counter += 1
