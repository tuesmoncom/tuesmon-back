...
from tuesmon.projects.models import Project, ProjectTemplate
from tuesmon.projects.userstories.models import UserStory
from tuesmon.projects.tasks.models import Task
from tuesmon.projects.attachments.models import Attachment
from tuesmon.projects.history.services import take_snapshot
from tuesmon.projects.history.models import HistoryEntry
from tuesmon.projects.custom_attributes.models import UserStoryCustomAttribute, TaskCustomAttribute
from tuesmon.users.models import User
from tuesmon.timeline.rebuilder import rebuild_timeline
from tuesmon.timeline.models import Timeline
from tuesmon.importers import exceptions
from tuesmon.importers import services as import_service
...
class AsanaImporter:
    ...
    def import_project(self, project_id, options):
        ...
        tuesmon_project = self._import_project_data(project, options)
        self._import_user_stories_data(tuesmon_project, project, options)
        Timeline.objects.filter(project=tuesmon_project).delete()
        rebuild_timeline(None, None, tuesmon_project.id)
        return tuesmon_project
    ...
    def _import_project_data(self, project, options):
        ...
        tuesmon_project = Project.objects.create(
        ...
        import_service.create_memberships(users_bindings, tuesmon_project, self._user, "asana")
    ...
    def _import_user_stories_data(self, tuesmon_project, project, options):
        users_bindings = options.get('users_bindings', {})
        ...
        due_date_field = tuesmon_project.userstorycustomattributes.first()
        ...
        for task in tasks:
            ...
            us = UserStory.objects.create(
                ...
                status=tuesmon_project.us_statuses.get(slug="closed" if task['completed'] else "open"),
            ...
            for subtask in subtasks:
                self._import_task_data(tuesmon_project, us, project, subtask, options)
    ...
    def _import_task_data(self, tuesmon_project, us, assana_project, task, options):
        users_bindings = options.get('users_bindings', {})
        tags = []
        for tag in task['tags']:
            tags.append(tag['name'].lower())
        due_date_field = tuesmon_project.taskcustomattributes.first()
        ...
        tuesmon_task = Task.objects.create(
            project=tuesmon_project,
            user_story=us,
            owner=self._user,
            assigned_to=assigned_to,
            status=tuesmon_project.task_statuses.get(slug="closed" if task['completed'] else "open"),
            us_order=task['id'],
        ...
        if task['due_on']:
            tuesmon_task.custom_attributes_values.attributes_values = {due_date_field.id: task['due_on']}
            tuesmon_task.custom_attributes_values.save()

        for follower in task['followers']:
            follower_user = users_bindings.get(follower['id'], None)
            if follower_user is not None:
                tuesmon_task.add_watcher(follower_user)

        Task.objects.filter(id=tuesmon_task.id).update(
            modified_date=task['modified_at'],
            created_date=task['created_at']
        )
        ...
        take_snapshot(tuesmon_task, comment="", user=None, delete=False)
        self._import_history(tuesmon_task, task, options)
        self._import_attachments(tuesmon_task, task, options)
