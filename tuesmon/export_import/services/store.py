...
from tuesmon.projects.history.services import make_key_from_model_object, take_snapshot
from tuesmon.projects.models import Membership
from tuesmon.projects.references import sequences as seq
from tuesmon.projects.references import models as refs
from tuesmon.projects.userstories.models import RolePoints
from tuesmon.projects.services import find_invited_user
from tuesmon.timeline.service import build_project_namespace
from tuesmon.users import services as users_service
...
def _validate_if_owner_have_enought_space_to_this_project(owner, data):
    ...
    if not enough_slots:
        raise err.TuesmonImportError(error_message, None)
...
def _create_project_object(data):
    ...
    if not project_validator:
        ...
        raise err.TuesmonImportError(_("error importing project data"), None, errors=errors)
...
def _populate_project_object(project, data):
    def check_if_there_is_some_error(message=_("error importing project data"), project=None):
        errors = get_errors(clear=True)
        if errors:
            raise err.TuesmonImportError(message, project, errors=errors)
...
def store_project_from_dict(data, owner=None):
    ...
    except err.TuesmonImportError:
        # raise known inport errors
        raise
    except Exception as e:
        # raise unknown errors as import error
        raise err.TuesmonImportError(_("unexpected error importing project"), project)
